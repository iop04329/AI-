...
def test():
    searchs:list[SearchResult] = google_searchAll(query='台灣00919 ETF')
    for i, e in enumerate(searchs):
        print(f'第{i}筆 標題:{e.title} link:{e.link}')
        content = get_stockArticleAll(url=e.link)
